...
class KppSyntaxError(Exception):
    def __init__(self, message: str, token: Token) -> None:
        self.code = ErrorCodes.SYNTAX
        self.kind = "Syntax Error"
        self.message = f"line {token.line}, token {token.lexeme!r}: {message}"
...
class Parser:
    ...
    def _parse_assignment(self):
        ...
        # set self attr to value.
        cur = self._current()
        nxt = self._peek(1)
        if cur.lexeme == "self" and nxt.kind == TK.IDENTIFIER:
            self._advance()  # self
            attr_name = self._expect_identifier()
    ...
    # ─────────────────────────────────────────────────────────────────────────
    # Import: import "file.kpp" .
    # ─────────────────────────────────────────────────────────────────────────

    def _parse_import(self):
        line = self._current().line
        self._expect_keyword("import")
        path_tok = self._expect_kind(TK.STRING, "Expected module path string after 'import'")
        self._expect_period()
        return ImportNode(line=line, module_path=path_tok.value)

    # ─────────────────────────────────────────────────────────────────────────
    # Define dispatch: define function ... | define class ...
    # ─────────────────────────────────────────────────────────────────────────

    def _parse_define(self):
        if self._next_is_keyword("function"):
            return self._parse_function_def()
        if self._next_is_keyword("class"):
            return self._parse_class_def()
        raise KppSyntaxError("Expected 'function' or 'class' after 'define'", self._current())

    # ─────────────────────────────────────────────────────────────────────────
    # Class def: define class NAME then define function ... end . end .
    # ─────────────────────────────────────────────────────────────────────────
    ...
    def _parse_class_def(self):
        ...
        while not self._check_keyword("end") and not self._check_kind(TK.EOF):
            if self._check_keyword("define") and self._next_is_keyword("function"):
                methods.append(self._parse_function_def())
                continue
            raise KppSyntaxError(
                "Class body supports only method definitions ('define function ...').",
                self._current(),
            )
        self._expect_keyword("end")
        self._expect_period()
        return ClassNode(line=line, name=name, methods=methods)
    ...
    def _parse_function_def(self):
        ...
        name = self._expect_identifier()
        params: List[str] = []
        if self._check_keyword("takes", "with"):
            self._advance()
    ...
    def _parse_call_stmt(self):
        ...
        if self._check_keyword("of"):
            self._advance()
            target_name = self._expect_identifier()
        args: List = []
        if self._check_keyword("with"):
            ...
            while self._check_kind(TK.COMMA):
                ...
                args.append(self._parse_expression())
        self._expect_period()
        return CallStmtNode(line=line, name=name, target_name=target_name, args=args)
    ...
    def _parse_expression(self):
        ...
        # ── new class instance ───────────────────────────────────────────────
        elif self._check_keyword("new"):
            left = self._parse_new_instance()
        # ── list of ... ──────────────────────────────────────────────────────
    ...
    def _parse_call_expr(self) -> CallExprNode:
        ...
        if self._check_keyword("with"):
            ...
            while self._check_kind(TK.COMMA):
                self._advance()
                args.append(self._parse_expression())
        return CallExprNode(line=line, name=name, target_name=target_name, args=args)
    ...
    def _parse_new_instance(self) -> NewInstanceNode:
        line = self._current().line
        ...
        if self._check_keyword("with"):
            ...
            while self._check_kind(TK.COMMA):
                self._advance()
                args.append(self._parse_expression())
        return NewInstanceNode(line=line, class_name=class_name, args=args)
    ...
    def _parse_primary(self):
        ...
        if t.kind == TK.IDENTIFIER:
            # self attr  -> self name
            if t.lexeme == "self" and self._peek(1).kind == TK.IDENTIFIER:
                self._advance()
                attr = self._expect_identifier()
                return AttrAccessNode(line=line, object_name="self", attr_name=attr)
            self._advance()
