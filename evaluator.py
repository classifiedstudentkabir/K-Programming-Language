...
class KppRuntimeError(Exception):
    ...
    def _format(self) -> str:
        msg = f"[K++ RuntimeError] at line {self.line}: {self.message}"
        if self.hint:
            msg += f"\n  Hint: {self.hint}"
        return msg
...
class Evaluator:
    ...
    def execute(self, node: ProgramNode) -> None:
    ...
    def _exec_add_to_list(self, node: AddToListNode, env: Environment) -> None:
        ...
        try:
            ...
            raise KppRuntimeError(f"List '{node.list_name}' is not defined.", node.line)
    ...
    def _call_function(self, name: str, args: List[Any], env: Environment, line: int) -> Any:
